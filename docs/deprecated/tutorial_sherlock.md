...
6. Run a pipeline for the test sample.
    ```bash
    $ sbatch --partition normal dev/examples/sherlock/ENCSR936XTK_subsampled_chr19_only_sherlock_conda.sh
    ```
...
6. Run a pipeline for the test sample.
    ```bash
    $ sbatch --partition normal dev/examples/sherlock/ENCSR936XTK_subsampled_chr19_only_sherlock_singularity.sh
    ```
