...
5. Run a pipeline for the test sample. You must have a paid account on SCG4.
    ```bash
    $ sbatch --account [YOUR_PAID_ACCOUNT_ON_SCG4] dev/examples/scg/ENCSR936XTK_subsampled_chr19_only_scg_conda.sh
    ```
...
4. Run a pipeline for the test sample. You must have a paid account on SCG4.
    ```bash
    $ sbatch --account [YOUR_PAID_ACCOUNT_ON_SCG4] dev/examples/scg/ENCSR936XTK_subsampled_chr19_only_scg_singularity.sh
    ```
