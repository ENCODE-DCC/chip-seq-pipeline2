...
11) Flags
    * `chip.redact_bam`: BAMs aligned from FASTQs will be redacted with [`ptools`](https://github.com/ENCODE-DCC/ptools). Variations will be replaced with corresponding reference sequence. **THIS FLAG IS FOR PIPELINE STARTING FROM FASTQS ONLY**. Starting from BAMs will deactivate this flag.
