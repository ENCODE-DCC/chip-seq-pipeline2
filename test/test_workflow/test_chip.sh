...
if [ $# -gt 1 ]; then
  ...
  DOCKER_IMAGE=quay.io/encode-dcc/chip-seq-pipeline:v1.1.1
fi
INPUT=$1
...
cat $PREFIX.result.json | python -c "import json,sys;obj=json.load(sys.stdin);print(obj['outputs']['chip.qc_json_match'])" > $PREFIX.result.qc_json_match.txt
